...
<script type="text/javascript">
	function approve(midtermCheckId){
		var confirmed = confirm("确认通过？");
		if(confirmed==true){
			$.ajax({
	...
	function notApprove(midtermCheckId){
		var confirmed = confirm("确认不通过？");
		if(confirmed==true){
			$.ajax({
			...
			<table class="table">
				...
				<tbody>
					<c:if test="${empty midtermCheckList }">
						<h2>
							<span class="label label-default">未有学生上传中期检查表，请耐心等待。</span>
						</h2>
					</c:if>
					<c:forEach items="${midtermCheckList}" var="midtermCheck">
						<tr class="midtermCheck${midtermCheck.id }">
							...
							<td class="midtermCheckdata${midtermCheck.id }">${midtermCheck.updateDate }</td>
							<td class="midtermCheckaudit${midtermCheck.id }"><c:if
									test="${empty midtermCheck.approve }">
								...
								<div class="btn-group">
									...
									<ul class="dropdown-menu" role="menu">
										<li>
											...
											onclick="approve(${midtermCheck.id})">通过</a>
										</li>
										<li>
											<%-- <a
											...
											onclick="notApprove(${midtermCheck.id})">不通过</a>
										</li>
									</ul>
